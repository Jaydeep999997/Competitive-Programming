...
mod_int ncr(int n, int r)
{
	...
	mod_int ans = (ifact[r] * ifact[n - r]);
	ans = (ans * fact[n]);
	return ans;
}
 
mod_int incr(int n, int r)
{
	if (r > n || n < 0 || r < 0)
	{
		return 0;
	}
	mod_int ans = (fact[r] * ifact[n - r]);
	ans = (ans * ifact[n]);
	return ans;
}
 
mod_int npr(int n, int r)
{
	if (r > n || n < 0 || r < 0)
	{
		return 0;
	}
	mod_int ans = ifact[n - r] * fact[n];
	return ans;
}
 
mod_int inpr(int n, int r)
{
	if (r > n || n < 0 || r < 0)
	{
		return 0;
	}
	mod_int ans = (fact[n - r] * ifact[n]);
	return ans;
}
